...
export function ProjectForm({ onCancel }: { onCancel?: () => void }) {
  ...
  const [search, setSearch] = useState<SearchSettingsValue>({
    ...
    // Empty: follow the country unless the user picks one.
    googleDomain: '',
  });
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      // Omitted rather than sent empty, so the server applies the country's.
      ...(search.googleDomain ? { googleDomain: search.googleDomain } : {}),
    };

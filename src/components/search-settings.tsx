...
export type SearchSettingsValue = {
  ...
  language: string;
  devices: string[];
  /** Empty means "follow the country", which is the default. */
  googleDomain: string;
};
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              id={`${idPrefix}-google`}
              name="googleDomain"
              value={value.googleDomain}
              onChange={(event) => onChange({ ...value, googleDomain: event.target.value })}
            >
              <option value="">
                Follow the country ({googleDomainFor(value.country)})
              </option>
            ...
            <p className="text-xs text-muted-foreground">
              Which Google answers. The country sets where the searcher is; these usually agree,
              but the results can differ, so you can pick one deliberately.
            </p>
          </div>
        </div>
      </fieldset>

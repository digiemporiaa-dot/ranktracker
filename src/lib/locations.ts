...
import { GOOGLE_DOMAINS, getCountry, type CountryCode } from '@/config/serp';
...
export async function resolveLocation(
  input: {
    ...
    city?: string | null;
    /**
     * Explicit Google property. Omit to use the country's local one, which is
  ...
  // An explicit choice wins; otherwise the country decides. Validation has
  // already restricted the value, so an unknown one here can only be stale
  // data, and falling back is safer than searching a domain we do not know.
  const googleDomain =
    input.googleDomain && GOOGLE_DOMAINS.includes(input.googleDomain)
      ? input.googleDomain
      : country.googleDomain;
  ...
  // changed, so there is nothing to look up. The Google domain is resolved
  // above rather than read from the stored row, so a project saved before

...
describeIf('location targeting and device tracking (integration)', () => {
  ...
  describe('creating a project', () => {
    ...
    it('ignores a location id sent by the caller', async () => {
      ...
      // The id is looked up from the country, never taken from the request.
      expect(data.project.locationCode).toBe(INDIA);
      expect(data.project.googleDomain).toBe('google.co.in');
    });

    it('honours an explicit Google property, but only a supported one', async () => {
      // Unlike the location id, this is a real choice: google.com and
      // google.co.in can answer the same India-located query differently.
      const chosen = await createProject({
        name: 'Explicit Google',
        domain: DOMAIN,
        country: 'IN',
        googleDomain: 'google.com',
      });

      expect(chosen.data.project.googleDomain).toBe('google.com');
      // The location is still India's, so only the Google property moved.
      expect(chosen.data.project.locationCode).toBe(INDIA);

      const forged = await createProject({
        name: 'Forged Google',
        domain: DOMAIN,
        country: 'IN',
        googleDomain: 'google.evil.com',
      });

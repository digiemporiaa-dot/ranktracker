...
describe('location validation', () => {
  ...
  // The Google property used to be stripped like the location ids are. It is
  // now a real choice, because google.com and google.co.in can answer the same
  // query differently and which one is right depends on the site. It stays
  // restricted to the supported list: an arbitrary domain is now rejected
  // outright rather than silently ignored.
  it('accepts a Google property only from the supported list', () => {
    const allowed = createProjectSchema.safeParse({
      ...base,
      country: 'IN',
      googleDomain: 'google.co.in',
    });
    expect(allowed.success && allowed.data.googleDomain).toBe('google.co.in');

    for (const domain of ['example.com', 'google.evil.com', 'bing.com', '']) {
      expect(
        createProjectSchema.safeParse({ ...base, country: 'IN', googleDomain: domain }).success,
        domain,
      ).toBe(false);
    }
  });

  it('still takes no Google property as "use the country\u2019s"', () => {
    const parsed = createProjectSchema.safeParse({ ...base, country: 'IN' });
    expect(parsed.success).toBe(true);
    if (parsed.success) expect(parsed.data.googleDomain).toBeUndefined();
  });
